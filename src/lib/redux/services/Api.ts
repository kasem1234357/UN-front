...
export const Api = createApi({
  ...
  endpoints: (builder) => ({
    getAllKeys:builder.query({
        query: ({limit=1000,currentPage=1}) =>
        `/api/rating/keysForFront?limit=${limit}&page=${currentPage}`,
    ...
    getAllStatics:builder.query({
     ...
     getAllEnginersStatics:builder.query({
        query: ({limit=10,currentPage=1,userName=""}) =>
        `/api/rating/engineers?limit=${limit}&page=${currentPage}&userName=${userName}`,
      providesTags:['storage']
